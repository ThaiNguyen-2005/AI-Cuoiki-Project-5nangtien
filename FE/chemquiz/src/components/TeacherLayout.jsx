...
const navItems = [
  ...
  { path: "/teacher/auto-quiz", label: "Tạo đề tự động", icon: "bolt" },
  { path: "/teacher/manual-quiz", label: "Tạo đề thủ công", icon: "edit_note" },
  { path: "/teacher/quiz", label: "Danh sách đề thi", icon: "quiz" },
  { path: "/teacher/question", label: "Ngân hàng câu hỏi", icon: "database" },
  { path: "/teacher/result", label: "Kết quả & Phân tích", icon: "analytics" },
  { path: "/teacher/profile", label: "Hồ sơ", icon: "account_circle" },
];
...
export default function TeacherLayout() {
  ...
  const NavLinks = ({ onClose }) => (
    <nav className="flex flex-col gap-1.5 p-4 flex-1 overflow-y-auto scrollbar-hide">
      <div className="text-[12px] font-bold text-gray-500 uppercase tracking-widest mb-2 px-4">Menu Giáo Viên</div>
      {navItems.map((item) => (
        <NavLink
          key={item.path}
          to={item.path}
  ...
  const SidebarHeader = () => (
    <div className="p-6 mb-2">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 shrink-0 rounded-xl bg-linear-to-br from-teal-500 to-emerald-600 flex items-center justify-center shadow-lg shadow-teal-500/20">
        ...
        <div className="flex flex-col" style={{ paddingTop: '10px' }}>
          ...
          <span className="text-[11px] font-bold text-teal-500 uppercase tracking-widest mt-1">Teacher Portal</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#050810] flex text-gray-200">
      {/* Background Decor */}
      <div className="fixed top-0 right-0 w-[500px] h-[500px] bg-teal-600/5 blur-[120px] pointer-events-none" />
      <div className="fixed bottom-0 left-60 w-[400px] h-[400px] bg-emerald-600/5 blur-[100px] pointer-events-none" />
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-[#0b1326]/50 backdrop-blur-xl border-r border-white/5 h-screen shrink-0 sticky top-0 overflow-hidden">
        ...
        <div className="p-4 border-t border-white/5 bg-white/2">
          <div className="flex items-center gap-3 px-4 py-3 mb-2 cursor-pointer hover:bg-white/5 rounded-2xl transition-colors" onClick={() => navigate('/profile')}>
            <div className="w-8 h-8 rounded-full bg-linear-to-br from-teal-500 to-emerald-600 flex items-center justify-center text-xs font-bold text-white border border-white/10 shadow-lg shadow-teal-500/10">
              {user?.name?.charAt(0) || 'T'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold text-white truncate">{user?.name || 'Giáo viên'}</p>
              <p className="text-[10px] text-gray-500 truncate capitalize">{user?.role || 'teacher'}</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative h-screen overflow-hidden lg:p-4">
        ...
        <header className="lg:hidden sticky top-0 z-30 bg-[#0b1326]/80 backdrop-blur-md border-b border-white/5 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-white tracking-tight">Kinetic Teacher</span>
          </div>
          <button
