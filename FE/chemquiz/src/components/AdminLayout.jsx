...
export default function AdminLayout() {
  ...
  const NavLinks = ({ onClose }) => (
    <nav className="flex flex-col gap-1.5 p-4 flex-1 overflow-y-auto">
      <div className="text-[12px] font-bold text-gray-500 uppercase tracking-widest mb-2 px-4">Menu</div>
      {navItems.map((item) => (
        <NavLink
          key={item.path}
          to={item.path}
    ...
    <div className="p-6 mb-2">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 shrink-0 rounded-xl bg-linear-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-blue-500/20">
          <span className="material-symbols-outlined text-white text-2xl font-light">science</span>
        </div>
        <div className="flex flex-col" style={{ paddingTop: '10px' }}>
          ...
          <span className="text-[11px] font-bold text-blue-500 uppercase tracking-widest mt-1">Admin Portal</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#050810] flex text-gray-200">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-[#0b1326]/50 backdrop-blur-xl border-r border-white/5 h-screen shrink-0 sticky top-0 overflow-hidden">
        ...
        <div className="p-4 border-t border-white/5 bg-white/2">
          <div className="flex items-center gap-3 px-4 py-3 mb-2 cursor-pointer hover:bg-white/5 rounded-2xl transition-colors" onClick={() => navigate('/admin/profile')}>
            <div className="w-9 h-9 rounded-full bg-linear-to-br from-gray-700 to-gray-800 flex items-center justify-center text-sm font-bold text-blue-400 border border-white/10 shrink-0">
              {user?.name?.charAt(0) || 'A'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-white truncate">{user?.name || 'Administrator'}</p>
              <p className="text-xs text-gray-500 truncate">{user?.email || 'admin@kinetic.io'}</p>
            </div>
          </div>
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative z-50 w-72 bg-[#0b1326] h-full shadow-2xl flex flex-col border-r border-white/10 animate-in slide-in-from-left duration-300">
            ...
            <div className="p-4 border-t border-white/5 mt-auto bg-white/2">
              <div className="flex items-center gap-3 px-4 py-3 mb-2" onClick={() => navigate('/admin/profile')}>
                <div className="w-9 h-9 rounded-full bg-linear-to-br from-gray-700 to-gray-800 flex items-center justify-center text-sm font-bold text-blue-400 border border-white/10 shrink-0">
                  {user?.name?.charAt(0) || 'A'}
                </div>
                <div className="flex-1 min-w-0">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative h-screen overflow-hidden lg:p-4">
        ...
        <header className="lg:hidden sticky top-0 z-30 bg-[#0b1326]/80 backdrop-blur-md border-b border-white/5 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-white tracking-tight">Kinetic Admin</span>
          </div>
          <button
        ...
        {/* Content Area Container (The "Outer Frame") */}
