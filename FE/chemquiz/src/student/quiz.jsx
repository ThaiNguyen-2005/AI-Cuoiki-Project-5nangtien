...
export default function StudentQuiz() {
  const [activeQuestion, setActiveQuestion] = useState(12); // Đang để mặc định câu 12 theo thiết kế của ông
  const [selectedOption, setSelectedOption] = useState(null);
  
  const navigate = useNavigate();
  const totalQuestions = 20;

  // Hàm xử lý khi bấm nút Nộp bài sớm
  const handleSubmitQuiz = () => {
    const isConfirm = window.confirm("Bạn có chắc chắn muốn nộp bài sớm không?");
    if (isConfirm) {
  ...
  return (
    <>
      ...
      <main className="pt-24 pb-32 px-4 md:px-10 max-w-6xl mx-auto">
          ...
          <div className="flex justify-between items-end mb-4">
            <div>
              <h2 className="font-headline-lg text-headline-lg text-primary mb-1">
                ...
                <span className="text-outline text-lg font-normal">/ {totalQuestions}</span>
              </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            {/* Lưới đáp án được map từ mảng */}
            {[
              { id: 'A', text: 'Hình tam giác đều, lai hóa sp²' },
              { id: 'B', text: 'Hình tứ diện đều, lai hóa sp³' },
              { id: 'C', text: 'Hình chóp tam giác, lai hóa sp³' },
              { id: 'D', text: 'Hình đường thẳng, lai hóa sp' }
            ].map((opt, idx) => (
              <button 
                key={idx}
                onClick={() => setSelectedOption(idx)}
                className={`group flex items-center p-6 rounded-2xl text-left relative transition-all duration-300 active:scale-[0.98] ${
                  selectedOption === idx 
                    ? 'bg-primary-container/20 border-2 border-primary-container shadow-[0_0_20px_rgba(192,193,255,0.15)]' 
                    : 'glass-card hover:bg-primary-container/10 border-2 border-transparent hover:border-primary-container/30'
                }`}
              >
                <span className={`w-10 h-10 rounded-xl flex items-center justify-center font-display font-bold mr-5 transition-colors ${
                  selectedOption === idx 
                    ? 'bg-primary-container text-on-primary-container' 
                    : 'bg-surface-container-highest text-primary group-hover:bg-primary-container group-hover:text-on-primary-container'
                }`}>
                  {opt.id}
                </span>
                <span className={`font-body-md text-lg ${
                  selectedOption === idx ? 'text-primary font-semibold' : 'text-on-surface'
                }`}>
                  {opt.text}
                </span>
                {/* Dấu check chỉ hiện khi được chọn */}
                {selectedOption === idx && (
                  <span
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <button 
                ...
                className="flex items-center justify-center gap-2 py-4 rounded-2xl bg-surface-container-high border border-outline-variant text-on-surface hover:bg-surface-variant transition-all font-display font-bold uppercase tracking-wider"
              >
              ...
              <button 
                ...
                className="flex items-center justify-center gap-2 py-4 rounded-2xl kinetic-gradient text-on-primary-container hover:brightness-110 transition-all font-display font-bold uppercase tracking-wider shadow-lg shadow-primary-container/20"
              >
                Tiếp theo
            ...
            {/* NÚT NỘP BÀI ĐÃ GẮN SỰ KIỆN */}
            <button 
              onClick={handleSubmitQuiz}
              className="mt-2 w-full py-4 rounded-2xl border-2 border-dashed border-tertiary-container/30 text-tertiary hover:bg-tertiary-container/10 transition-all font-display font-bold uppercase tracking-widest text-sm"
            >
              Nộp bài sớm
            </button>
          </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-2 pb-safe py-3 bg-[#0b1326]/90 backdrop-blur-xl border-t border-indigo-500/30 shadow-[0_-4px_20px_rgba(192,193,255,0.1)]">
        <a className="flex flex-col items-center justify-center text-slate-500 hover:text-slate-300 px-4 py-1.5 transition-colors" href="#">
          <span className="material-symbols-outlined">home</span>
          <span className="font-['Space_Grotesk'] text-[10px] font-medium">Home</span>
        </a>
        <a className="flex flex-col items-center justify-center text-[#c0c1ff] bg-indigo-500/20 rounded-xl px-4 py-1.5 brightness-125 scale-105" href="#">
          <span className="material-symbols-outlined">quiz</span>
          <span className="font-['Space_Grotesk'] text-[10px] font-medium">Quiz</span>
        </a>
        <a className="flex flex-col items-center justify-center text-slate-500 hover:text-slate-300 px-4 py-1.5 transition-colors" href="#">
          <span className="material-symbols-outlined">history</span>
          <span className="font-['Space_Grotesk'] text-[10px] font-medium">History</span>
        </a>
        <a className="flex flex-col items-center justify-center text-slate-500 hover:text-slate-300 px-4 py-1.5 transition-colors" href="#">
          <span className="material-symbols-outlined">insights</span>
          <span className="font-['Space_Grotesk'] text-[10px] font-medium">Analysis</span>
        </a>
      </nav>
      ...
      <aside className="hidden md:flex fixed left-0 top-16 h-[calc(100vh-4rem)] w-20 flex-col items-center py-8 gap-10 bg-surface-container-lowest border-r border-outline-variant/10">
        <div className="relative group">
          <span className="material-symbols-outlined text-outline group-hover:text-primary transition-colors cursor-pointer">home</span>
        ...
        <div className="relative group">
          <span className="material-symbols-outlined text-outline group-hover:text-primary transition-colors cursor-pointer">history</span>
        </div>
        <div className="relative group">
          <span className="material-symbols-outlined text-outline group-hover:text-primary transition-colors cursor-pointer">insights</span>
        </div>
        <div className="mt-auto relative group">
          <span className="material-symbols-outlined text-outline group-hover:text-primary transition-colors cursor-pointer">settings</span>
        </div>
      </aside>
    </>
  );
